...
const WeeklyMenu: React.FC<{
  ...
  return (
    <div className="flex flex-col h-full px-4 pt-8">
      {/* Intestazione sticky senza sfondo visibile */}
      <div className="sticky top-0 z-10 backdrop-blur-sm pb-4">
        ...
        <div className="flex justify-center gap-4">
          <motion.button
            className="bg-white text-purple-600 font-semibold px-4 py-2 text-sm rounded-lg shadow hover:bg-gray-200"
      ...
      {/* Cards scrollabili */}
      <div className="overflow-y-auto mt-6 pb-6 pr-2 flex-grow">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {Object.entries(menu).map(([giorno, pasti], index) => (
            <motion.div
              key={giorno}
              className={`bg-white bg-opacity-20 backdrop-blur-md rounded-2xl shadow-lg p-6 border border-white/20 transition-transform hover:scale-105 bg-gradient-to-br ${
